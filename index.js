...
get(child(ref(db), "events/" + eventID)).then((snapshot) => {
	if (snapshot.exists()) {
		for (attendee in snapshot.val()) {
			console.log(snapshot.val()[attendee]);
			...
			generatePass(snapshot.val()[attendee].firstName, snapshot.val()[attendee].lastName, snapshot.val()[attendee].id, serial);
		}
	} else {
...
async function generatePass(firstName, lastName, id, serial) {
	...
	fs.writeFile("./passes/" + serial + ".pkpass", buf, () => { });
}